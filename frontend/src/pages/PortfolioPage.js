...
const PROJECTS = [
  {
    ...
    desc: 'Professional corporate website demo — Home, Services, About and Contact pages with custom branding, color system and fully responsive layout.',
    stack: ['React', 'Node.js', 'Tailwind CSS', 'Framer Motion'],
    results: ['Professional brand identity', 'Fully responsive layout', 'Custom color system'],
  ...
  },
  {
    ...
    results: ['Product filter & search', 'Stripe payment integration', 'Mobile-first design'],
  ...
  },
  {
    ...
    desc: 'SaaS POS dashboard demo — login, analytics charts, reports, user management and a fully responsive admin UI built for real business workflows.',
    stack: ['React', 'Chart.js', 'Node.js', 'MongoDB', 'JWT'],
    results: ['Role-based access control', 'Sales analytics dashboard', 'Inventory management'],
    liveUrl: 'https://demo-pos.axentralab.com',
    featured: true,
    isDemo: true,
  },
  {
    id: 'd4',
    title: 'SwiftRoute — Courier Platform',
    tag: 'Custom System',
    category: 'Demo',
    color: '#EF4444',
    year: '2025',
    thumbnail: '/images/portfolio/demo-courier.jpg',
    desc: 'Courier management system demo — parcel tracking, delivery status, rider management, client portal and admin dashboard in one platform.',
    stack: ['React', 'Node.js', 'MongoDB', 'Google Maps API'],
    results: ['Live parcel tracking', 'Rider management system', 'Client + admin portal'],
    liveUrl: '#',
    featured: false,
    isDemo: true,
  },
  {
    id: 'd5',
    title: 'Axentralab — Agency Website',
    tag: 'Corporate Website',
    category: 'Demo',
    color: '#06B6D4',
    year: '2025',
    thumbnail: '/images/portfolio/axentralab-site.jpg',
    desc: 'Our own agency website — built to showcase what we can do. Full MERN stack, dark UI, animations, blog, shop, dashboard and admin panel.',
    stack: ['React', 'Node.js', 'MongoDB', 'Stripe', 'JWT'],
    results: ['Full blog system', 'Service shop + checkout', 'Admin dashboard'],
    liveUrl: 'https://axentralab.com',
    featured: false,
    isDemo: false,
  },
];
...
const TESTIMONIALS = [
  { name: 'A Happy Client',  role: 'Business Owner, Dhaka',    avatar: 'AH', color: '#22C55E', quote: 'Axentralab delivered a clean, professional website for our business on time and within budget. Great communication throughout.' },
  { name: 'Local Client',    role: 'E-commerce Store Owner',   avatar: 'LC', color: '#3B82F6', quote: 'They built our online shop from scratch. The design is modern and the checkout works perfectly. Highly recommend.' },
  { name: 'Startup Founder', role: 'Tech Startup, Bangladesh', avatar: 'SF', color: '#A855F7', quote: 'The team understood our requirements quickly and delivered a solid custom system. Will definitely work with them again.' },
];
        ...
        <div style={{ padding:'0 32px 28px', display:'flex', gap:10, flexWrap:'wrap' }}>
          {project.liveUrl && project.liveUrl !== '#' ? (
            <a href={project.liveUrl} target="_blank" rel="noopener noreferrer"
              style={{ flex:1, padding:'13px', background:color, color:['#22C55E','#F59E0B','#06B6D4'].includes(color)?'#000':'#fff', border:'none', borderRadius:12, fontSize:14, fontWeight:700, fontFamily:"'Sora',sans-serif", textDecoration:'none', textAlign:'center', minWidth:140 }}>
...
function ProjectCard({ project, index, onOpenModal }) {
  ...
  return (
    <div
      onClick={() => onOpenModal(project)}
      style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:20, overflow:'hidden', transition:'all 0.3s', animation:`fadeUp 0.5s ${Math.min(index,8)*0.06}s ease both`, display:'flex', flexDirection:'column', cursor:'pointer' }}
      onMouseEnter={e => { e.currentTarget.style.borderColor=`${project.color}40`; e.currentTarget.style.transform='translateY(-6px)'; e.currentTarget.style.boxShadow=`0 20px 48px ${project.color}14`; }}
    ...
    >
      ...
      <div style={{ padding:'18px 20px 20px', display:'flex', flexDirection:'column', flex:1 }}>
        ...
        <h3 style={{ fontFamily:"'Sora',sans-serif", fontSize:16, fontWeight:900, color:'#fff', margin:'0 0 8px', letterSpacing:-0.3 }}>{project.title}</h3>
    ...
    <div style={{ padding:'108px 5% 0', minHeight:'100vh' }}>
      ...
      <div style={{ textAlign:'center', marginBottom:64, animation:'fadeUp 0.6s ease both' }}>
        <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'5px 16px', borderRadius:999, border:'1px solid rgba(34,197,94,0.3)', background:'rgba(34,197,94,0.07)', marginBottom:20 }}>
          ...
          <span style={{ fontFamily:"'Space Mono',monospace", fontSize:11, color:'#22C55E', letterSpacing:1, textTransform:'uppercase', fontWeight:600 }}>Our Work</span>
        </div>
        ...
        <p style={{ fontFamily:"'Sora',sans-serif", fontSize:'clamp(14px,1.8vw,17px)', color:'rgba(255,255,255,0.42)', maxWidth:500, margin:'0 auto 32px', lineHeight:1.8 }}>
          Axentralab is a results-driven digital agency helping businesses scale with modern web solutions, smart systems, and secure technology.
        </p>
        <div style={{ display:'flex', gap:20, justifyContent:'center', flexWrap:'wrap' }}>
          {['🔒 NDA available','📦 Full source code','🚀 On-time delivery','📍 Based in Dhaka'].map((v,i) => (
      ...
      <div className="filter-row">
        ...
        <div style={{ position:'relative' }}>
          <select value={activeStack} onChange={e => handleStack(e.target.value)}
            ...
            {ALL_STACKS.map(s => <option key={s} value={s} style={{ background:'#0a0c14', color:'#fff' }}>{s}</option>)}
          </select>
        ...
        {(search || activeStack || activeCategory !== 'All') && (
          <div style={{ fontSize:12, color:'rgba(255,255,255,0.3)', fontFamily:"'Space Mono',monospace", padding:'8px 14px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:10 }}>
            ...
            <button onClick={() => { handleSearch(''); handleStack(''); handleCategoryChange('All'); }} style={{ marginLeft:10, color:'rgba(255,255,255,0.3)', background:'none', border:'none', cursor:'pointer', fontSize:11, textDecoration:'underline' }}>clear</button>
          </div>
        )}
      </div>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign:'center', padding:'60px 20px 80px' }}>
          ...
          <p style={{ color:'rgba(255,255,255,0.4)', fontSize:15, marginBottom:20 }}>No projects found.</p>
          <button onClick={() => { handleSearch(''); handleStack(''); handleCategoryChange('All'); }} style={{ padding:'10px 22px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, color:'#fff', fontSize:13, cursor:'pointer' }}>Show All</button>
        </div>
      )}
        ...
        <section style={{ marginBottom:48 }}>
          ...
          {hasMore && (
            <div style={{ textAlign:'center', marginTop:40 }}>
              <button onClick={() => setVisible(v => v + PER_LOAD)}
      ...
      <section style={{ maxWidth:1100, margin:'0 auto 88px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:22, padding:'clamp(32px,5vw,52px) clamp(24px,4vw,48px)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:16 }}>
          {[
            { step:'01', icon:'💬', title:'Discovery Call',    desc:'We discuss your requirements, timeline and budget. Free consultation, no obligation.' },
            { step:'02', icon:'📐', title:'Clear Proposal',    desc:'You receive a fixed-price quote with scope, timeline and deliverables — no hidden costs.' },
            { step:'03', icon:'⚙️', title:'Build & Review',    desc:'We build in stages and share progress regularly. Your feedback shapes the final product.' },
            { step:'04', icon:'🚀', title:'Launch & Handover', desc:'We deploy your project, hand over all source code and provide post-launch support.' },
          ].map((s,i) => (
      ...
      <section style={{ marginBottom:88 }}>
        <div style={{ textAlign:'center', marginBottom:44 }}>
          ...
          <h2 style={{ fontFamily:"'Sora',sans-serif", fontSize:'clamp(22px,3.5vw,36px)', fontWeight:900, color:'#fff', marginTop:12, letterSpacing:-0.8 }}>What Clients Say</h2>
        </div>
      ...
      <section style={{ padding:'36px 0', borderTop:'1px solid rgba(255,255,255,0.05)', borderBottom:'1px solid rgba(255,255,255,0.05)', overflow:'hidden', marginBottom:88 }}>
        <div style={{ display:'flex', gap:48, animation:'marquee 22s linear infinite', width:'max-content' }}>
          {[...['React','Node.js','MongoDB','Stripe','Figma','Chart.js','JWT','Google Maps API','Tailwind CSS','Framer Motion'],
            ...['React','Node.js','MongoDB','Stripe','Figma','Chart.js','JWT','Google Maps API','Tailwind CSS','Framer Motion']].map((t,i) => (
        ...
        <div style={{ position:'relative', background:'linear-gradient(135deg,rgba(34,197,94,0.09) 0%,rgba(59,130,246,0.06) 100%)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:24, padding:'clamp(48px,6vw,80px) clamp(24px,5%,64px)', textAlign:'center', overflow:'hidden' }}>
          ...
          <div style={{ position:'relative' }}>
            ...
            <h2 style={{ fontFamily:"'Sora',sans-serif", fontSize:'clamp(26px,4.5vw,52px)', fontWeight:900, color:'#fff', margin:'18px auto 16px', letterSpacing:-1.5, lineHeight:1.08, maxWidth:580 }}>
              Ready to Build<br /><span style={{ color:'#22C55E' }}>Something Real?</span>
            </h2>
            ...
            <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap', marginBottom:24 }}>
              ...
              <Link to="/contact" style={{ padding:'15px 28px', fontSize:15, borderRadius:12, textDecoration:'none', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.12)', color:'#fff', fontFamily:"'Sora',sans-serif", fontWeight:700, display:'inline-block' }}>
                Get a Free Quote
              </Link>
            </div>
